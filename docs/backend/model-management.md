...
异步加载采用**单一动态超时**，根据模型文件大小自动计算：

- 公式：`timeout = clamp(sizeGB × 1min, 5min, 30min)`
- 最小 5 分钟，最大 30 分钟，每 GB 约 1 分钟

健康检查 goroutine **不设独立总超时**，仅负责：
1. 检查进程存活（进程意外退出则报错）
2. 轮询后端 `/health` 端点（HTTP 200 = 就绪，503/连接失败 = 仍在加载）
3. 检测进程输出中的加载完成信号（如 `"all slots are idle"`）

超时统一由外层 `time.After(timeout)` 控制，避免大模型（如 Q8_0 量化模型）在 tensor 加载阶段被过早终止。
...
| 异步加载 + 单一动态超时 | 大模型加载耗时不固定，动态超时（5-30min，基于模型大小）避免"一刀切"。健康检查不设独立超时，防止双重超时竞态导致大模型被误杀 |
