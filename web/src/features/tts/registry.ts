import type { LoadedModel } from '@/features/creative/hooks';
import type { TTSPlugin } from './types';

/**
 * Registry for TTS plugins. Plugins register themselves on import,
 * and the page shell queries the registry to determine which tabs to show.
 */
class TTSPluginRegistry {
  private plugins: TTSPlugin[] = [];

  /**
   * Register a TTS plugin. Plugins with lower `order` appear first in the tab bar.
   */
  register(plugin: TTSPlugin): void {
    // Avoid duplicate registration
    if (this.plugins.some((p) => p.id === plugin.id)) return;
    this.plugins.push(plugin);
    this.plugins.sort((a, b) => (a.order ?? 100) - (b.order ?? 100));
  }

  /**
   * Get the best-matching plugin for a given model.
   * Returns the first plugin (by order) whose `match()` returns true,
   * excluding the 'generic' fallback.
   */
  getPluginForModel(model: LoadedModel): TTSPlugin | null {
    for (const plugin of this.plugins) {
      if (plugin.id === 'generic') continue;
      if (plugin.match(model)) return plugin;
    }
    return this.getGenericPlugin();
  }

  /**
   * Get the generic fallback plugin.
   */
  getGenericPlugin(): TTSPlugin | null {
    return this.plugins.find((p) => p.id === 'generic') ?? null;
  }

  /**
   * Get all registered plugins in display order.
   */
  getAllPlugins(): TTSPlugin[] {
    return [...this.plugins];
  }

  /**
   * Get models matching a specific plugin from a list of loaded TTS models.
   */
  getModelsForPlugin(plugin: TTSPlugin, allTTSModels: LoadedModel[]): LoadedModel[] {
    if (plugin.id === 'generic') {
      // Generic matches models that no other plugin claims
      return allTTSModels.filter((m) => {
        const specific = this.plugins.find((p) => p.id !== 'generic' && p.match(m));
        return !specific;
      });
    }
    return allTTSModels.filter((m) => plugin.match(m));
  }
}

export const ttsRegistry = new TTSPluginRegistry();
