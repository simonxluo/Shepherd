...
WHERE ($1::text = '' OR backend_type = $1)
  AND ($2::text = '' OR model_scope = '' OR model_scope = $2)
ORDER BY name ASC
`

type ListLaunchProfilesParams struct {
	BackendTypeFilter string `json:"backend_type_filter"`
	ModelScopeFilter  string `json:"model_scope_filter"`
}
