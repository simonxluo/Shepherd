...
func TestShardedModelSizes(t *testing.T) {
	...
	for i, shard := range shards {
		...
		fmt.Printf("分卷 %d (%s):\n", i+1, shard)
		fmt.Printf("  os.Stat().Size(): %d 字节 (%.2f GB)\n", shardSizes[i], float64(shardSizes[i])/(1024*1024*1024))
		...
		// 验证 meta.FileSize 返回正确的大小
		if int64(meta.FileSize) != shardSizes[i] {
			t.Errorf("❌ 分卷 %d Meta.FileSize 不匹配: 得到 %d, 期望 %d", i+1, meta.FileSize, shardSizes[i])
		}
	}
}
